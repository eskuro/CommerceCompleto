...
namespace Servicios.Localidad
{
    public class LocalidadServicio : ILocalidadServicio
    {
        ...
        {
            Expression<Func<Dominio.Entidades.Localidad, bool>> filtro = x =>
        ...
        {
            return _unidadDeTrabajo.LocalidadRepositorio.Obtener(x => !x.EstaEliminado && x.DepartamentoId == departamentoId, "Departamento, Departamento.Provincia")
                .Select(x => new LocalidadDto
                {
                    Id = x.Id,
                    DepartamentoId = x.DepartamentoId,
