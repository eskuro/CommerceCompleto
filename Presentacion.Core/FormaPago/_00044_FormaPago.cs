...
namespace Presentacion.Core.FormaPago
{
	public partial class _00044_FormaPago : FormBase
    {
        ...
        public _00044_FormaPago()
		{
            ...
            PoblarComboBox(cmbBan, _bancoServicio.Obtener(string.Empty, false), "Descripcion", "Id");
            PoblarComboBox(cmbTar, _tarjetaServicio.Obtener(string.Empty, false), "Descripicion", "Id");
		...
		private void btnBuscarCliente_Click(object sender, EventArgs e)
		{
            ...
            if (fLookUpCliente.EntidadSeleccionada != null)
            {
                ...
                if (_cliente.ActivarCtaCte)
                {
                    ...
                    if (_cliente.Dni != Aplicacion.Constantes.Cliente.ConsumidorFinal)
                    {
                        txtMontoAdeudado.Text = _cuentaCorrienteServicio.ObtenerDeudaCliente(_cliente.Id).ToString("C");
                    }
                    else
                    {
		...
		private void btnConfirmar_Click(object sender, EventArgs e)
		{
			try
			{
                if (_vieneDeVentas)
                {
                    ...
                    {
                        if (MessageBox.Show("El total que esta por abonar es superioral monto a pagar.Desea continuar ? ", "Atención", MessageBoxButtons.OKCancel,
                ...
                if (_vieneDeVentas)
                {
                    ...
                    _facturaNueva.PuestoTrabajoId = _factura.PuestoVentaId;
                ...
                if (nudTotalTarjeta.Value > 0)
                {
                    _facturaNueva.FormasDePagos.Add(new FormaPagoTarjetaDto
                    {
                        ...
                        TarjetaId = (long)cmbTar.SelectedValue,
                        Eliminado = false
                    });
                }
                ...
                if (nudTotalCheque.Value > 0)
                {
                    ...
                    {
                        BancoId = (long)cmbBan.SelectedValue,
                ...
                {
                    var deuda = _vieneDeVentas ?  _cuentaCorrienteServicio.ObtenerDeudaCliente(_factura.Cliente.Id)
                    ...
                    if (_vieneDeVentas)
                    {
                        ...
                        if (_factura.Cliente.ActivarCtaCte)
                        {
                            ...
                            {
                                var menssajeCtaCte =
                                                     $"El cliente {_factura.Cliente.ApyNom} esta por arriba del limite Permitido."
                                                   + Environment.NewLine
